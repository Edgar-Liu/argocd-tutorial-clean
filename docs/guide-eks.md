# Guide: EKS Cluster + ECR

A hands-on tutorial using an existing EKS cluster with AWS ECR for container images.
...
# If empty, re-export them from Steps 2-3
...
### 9a: Set Up AWS OIDC Provider (One-time per AWS Account)
...
cat > trust-policy.json << EOF
{
  ...
  "Statement": [{
    ...
    "Condition": {
      ...
      "StringLike": {
        "token.actions.githubusercontent.com:sub": "repo:$GITHUB_USERNAME/argocd-tutorial-clean:*"
      }
    }
  }]
}
...
aws iam create-role \
  --role-name GitHubActionsECRRole-$K8S_USERNAME \
...
aws iam get-role --role-name GitHubActionsECRRole-$K8S_USERNAME --query 'Role.Arn' --output text
```
...
aws ecr delete-repository --repository-name $ECR_REPO --region $AWS_REGION --profile raid-commonsvcs-prod --force

# Clean up IAM role (if created)
aws iam detach-role-policy --role-name GitHubActionsECRRole-$K8S_USERNAME --policy-arn arn:aws:iam::aws:policy/AmazonEC2ContainerRegistryPowerUser
aws iam delete-role --role-name GitHubActionsECRRole-$K8S_USERNAME
```
